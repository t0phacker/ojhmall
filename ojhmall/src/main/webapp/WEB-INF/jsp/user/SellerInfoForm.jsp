...
<body>
	<div class="container">
		...
		<!-- 판매자 회원 정보 표시, ID, Password, 이름, 전화번호, 주소, 우편번호, 계좌번호, 사업자 번호 -->
		<form id="frm" class="form-signin" method="post"
			action="changeSellerInfo.do">
			...
			<input type="hidden" id="userType" name="userType" class="form-control"
				placeholder="text" value="${sessionScope.userLogInInfo.userType}"
				required>
			<h4>ID</h4>
			<div class="well">
			...
			<label for="accNumber" class="sr-only">Account Number</label> <input
				type="text" id="accNumber" name="accNumber" class="form-control"
				placeholder="Zip Code"
				value="${sessionScope.userLogInInfo.accNumber
				}" required>
			<h4>Business Number</h4>
			<label for="bizNumber" class="sr-only">Business Number</label> <input
				type="text" id="bizNumber" name="bizNumber" class="form-control"
				placeholder="Zip Code"
