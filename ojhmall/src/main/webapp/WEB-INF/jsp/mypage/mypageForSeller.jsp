						...
						<c:choose>
							<c:when test="${fn:length(orderList) > 0 }">
								<c:forEach items="${orderList }" var="row" varStatus="status">
									<tr>
										<td width=15%>${row.orderNumber }<br>
										<td>${row.orderNewDate }</td>
										<td>${row.id }</td>
										<td>${row.price }</td>
										<td>${row.accNumber}</td>
										<td><c:if test="${row.orderStatus == 3 }">
												<h4>
										...
										<td><form action="checkOrder.do" method="POST">
												<input type="hidden" name="orderNumber"
													value="${row.orderNumber }">
												<c:if test="${row.orderStatus == 3 }">
													<h4>
														<input type="submit" class="btn btn-danger" value="결제확인"
															role="button">
													</h4>
												</c:if>
